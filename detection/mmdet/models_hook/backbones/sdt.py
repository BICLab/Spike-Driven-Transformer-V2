...
class SepConv(nn.Module):
    ...
    def forward(self, x, hook=None):
        ...
        x = self.lif1(x)
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_pw_lif'] = x.detach()

        x = self.bn1(self.pwconv1(x.flatten(0, 1))).reshape(T, B, -1, H, W)
        x = self.lif2(x)
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_dw_lif'] = x.detach()
...
class MS_ConvBlock(nn.Module):
    def __init__(
        ...
        self.Conv = SepConv(dim=dim, layer=layer)
    ...
    def forward(self, x, hook):
        ...
        x = self.lif1(x)
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_conv1_lif'] = x.detach()
        x = self.bn1(self.conv1(x.flatten(0, 1))).reshape(T, B, self.mlp_ratio * C, H, W)

        x = self.lif2(x)
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_conv2_lif'] = x.detach()
...
class MS_MLP(nn.Module):
    ...
    def forward(self, x, hook):
        ...
        x = self.fc1_lif(x)
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_fc1_lif'] = x.detach()
        x = self.fc1_conv(x.flatten(0, 1))
        x = self.fc1_bn(x).reshape(T, B, self.c_hidden, N).contiguous()

        x = self.fc2_lif(x)
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_fc2_lif'] = x.detach()
...
class MS_Attention_RepConv_qkv_id(nn.Module):
    ...
    def forward(self, x, hook):
        ...
        x = self.head_lif(x)
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_head_lif'] = x.detach()
        ...
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_q_lif'] = q.detach()
        ...
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_k_lif'] = k.detach()
        ...
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_v_lif'] = v.detach()

        x = k.transpose(-2, -1) @ v
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_kv'] = x.detach()

        x = (q @ x)
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_attn'] = x.detach()

        x = x.transpose(3, 4).reshape(T, B, C, N).contiguous()
        x = self.attn_lif(x).reshape(T, B, C, H, W)
        if hook is not None:
            hook[self._get_name() + '_layer_' + str(self.layer) + '_attn_lif'] = x.detach()
...
class MS_Block(nn.Module):
    def __init__(
        ...
        self.mlp = MS_MLP(in_features=dim, hidden_features=mlp_hidden_dim, drop=drop, layer=layer)
...
class MS_DownSampling(nn.Module):
    ...
    def forward(self, x, hook=None):
        ...
        if hasattr(self, "encode_lif"):
            x = self.encode_lif(x)
            if hook is not None:
                hook[self._get_name() + "_layer_" + str(self.layer) + "_lif"] = x.detach()
...
class Spiking_vit_MetaFormer(BaseModule):
    def __init__(
        ...
        self.ConvBlock1_1 = nn.ModuleList(
            [MS_ConvBlock(dim=embed_dim[0] // 2, mlp_ratio=mlp_ratios,layer=1)]
        )
        ...
        self.ConvBlock1_2 = nn.ModuleList(
            [MS_ConvBlock(dim=embed_dim[0], mlp_ratio=mlp_ratios,layer=2)]
        )
        ...
        self.ConvBlock2_1 = nn.ModuleList(
            [MS_ConvBlock(dim=embed_dim[1], mlp_ratio=mlp_ratios,layer=3)]
        )

        self.ConvBlock2_2 = nn.ModuleList(
            [MS_ConvBlock(dim=embed_dim[1], mlp_ratio=mlp_ratios,layer=4)]
        )
    ...
    def forward(self, x,hook = None):
        ...
        x,hook = self.forward_features(x,hook)
