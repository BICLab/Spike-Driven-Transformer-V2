...
class TwoStageDetector(BaseDetector):
    ...
    def predict(self,
        ...
        if batch_data_samples[0].get('proposals', None) is None: #输入是五个层的特征
            rpn_results_list = self.rpn_head.predict(
                x, batch_data_samples, rescale=False) #得到1000个类别得分，以及4000个回归框位置
        else:
            rpn_results_list = [
                data_sample.proposals for data_sample in batch_data_samples
            ]

        results_list = self.roi_head.predict(   #完成最终预测  专利写到这儿了
